function runExp3(noCST, noPST, noDT, noPDT, RANDOMIZE)
...
fmask_dur = nFrames - (TR(tr).pSOA + 1)/2;
...
for f = 1:nFrames
    ...
    if f >= f_start && f < (f_start + f_dur)
        ...
        if TR(tr).diskType == 1
            ...
            Screen('FillArc', Cfg.windowPtr, Cfg.green, [circ_left + 100,circ_up,circ_right + 100,circ_down], 0, 180); %Derecha + 100 - ¡Aquí!
            Screen('FillArc', Cfg.windowPtr, Cfg.red, [TR(tr).rectCirclePeriph], 180, 180); %Izquierda
        elseif TR(tr).diskType == 2
            Screen('FillArc', Cfg.windowPtr, Cfg.red,[circ_left + 100,circ_up,circ_right + 100,circ_down], 0, 180); %Derecha  - ¡Aquí!
    ...
    if f >= fmask_start && f < (fmask_start + fmask_dur)
        ...
        Screen('DrawTexture', Cfg.windowPtr, faceData, [], [circ_left + 100,circ_up,circ_right + 100,circ_down]); % - ¡Aquí!
