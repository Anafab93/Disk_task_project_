...
nTrials =10; % Set nTrials to 30; para la run 2, hay que poner "20" :), al terminar hay que volver a poner el número normal (30 o 48).
...
[Cfg.windowPtr, rect] = Screen('OpenWindow', Cfg.screenNumber,[100 100 100],Cfg.WinSize); %0 not [100 100 100]
...
%% SET UP PARAMETERS FOR DISK COLOURS
Screen('BlendFunction',Cfg.windowPtr,'GL_SRC_ALPHA','GL_ONE_MINUS_SRC_ALPHA');

Cfg.red_weight = 2;
Cfg.green_weight = 2;

Cfg.red = [(74 * Cfg.red_weight) 0 0];%I changed this, it used to be 207, now it is 100 -- now it is 50 (TOO DARK) -- now it is 75 - 65 -> back to 207
Cfg.green = [0 (43 * Cfg.green_weight) 0]; %It used to be 121, now it is 60 -- now it is 30 (TOO DARK) -- now it is 45 - 38 ->
Cfg.gray = [100 100 100];

%% CREATE TEXTURES OF LETTERS TO DISPLAY
Screen('BlendFunction',Cfg.windowPtr,'GL_SRC_ALPHA','GL_ONE_MINUS_SRC_ALPHA');
L_data = imread('Copy_of_faces/L.jpg');
L_texture = Screen('MakeTexture', Cfg.windowPtr, L_data);
% L_data(:, :, 4) = alpha; %pex

T_data = imread('Copy_of_faces/T.jpg');
T_texture = Screen('MakeTexture', Cfg.windowPtr, T_data);
% T_data(:, :, 4) = alpha;%pex


F_data = imread('Copy_of_faces/F.jpg');
...
disks = zeros(1,nTrials);
disks(1:nTrials/2) = 1;
disks(nTrials/2+1:end) = 2;
disks = Shuffle(disks);

PR = zeros(1,nTrials);
PR(1:nTrials/2) = 1;
PR(nTrials/2+1:end) = 2;
PR = Shuffle(PR);
...
for tr = 1 : nTrials
    ...
    TR(tr).imageHeightPeriph = imageHeightPeriph; %height of disk in periphery
    ...
    %Set red/green or green/red disk for main peripheral task
    
    TR(tr).diskType = disks(tr);
    
    
%   Nop nop nop  %% %Le agrego lo de BlendFunction, locoshón
% %Alpha blending mode
% Screen('BlendFunction',Cfg.windowPtr,'GL_SRC_ALPHA','GL_ONE_MINUS_SRC_ALPHA');
% Screen(Cfg.windowPtr,'BlendFunction',GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);
% 
% %Find the color values which correspond to white, gray and black.
% black = BlackIndex(Cfg.windowPtr);
% gray = GrayIndex(Cfg.windowPtr);
% white = WhiteIndex(Cfg.windowPtr);
%%
    %Set mondrian as mask
    %mondrians = randperm(300);
    TR(tr).gender_mask = randi(300); %mondrians(1); %1
    
    % Set face image number (here from 1 - 75) randomly (not needed in this
    % task)
    %TR(tr).picNo2_mask = '';
    TR(tr).picNo2_mask = '';
    ...
    cSOAGuess=30;
    cSOAGuessSd=20;
    pThreshold=0.7; %¡Lo voy a modificar! a 0.9, a ver qué paxa
