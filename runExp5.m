function runExp5(noCST, noPST, noDT, noPDT, RANDOMIZE)
...
for f = 1:nFrames
    ...
    if f >= f_start && f < (f_start + f_dur)
        ...
        diametro = (circ_down - circ_up); % Solo poner aquí 0. solo borrar esto si es de la otra forma, y también abajo.
        
        if TR(tr).diskType == 1
            TR(tr).rectCirclePeriph
            Screen('FillArc', Cfg.windowPtr, Cfg.green,[circ_left,circ_up - diametro - 100,circ_right,circ_down - diametro - 100], 0, 180); %Derecha + 100 - ¡Aquí!
            Screen('FillArc', Cfg.windowPtr, Cfg.red,[TR(tr).rectCirclePeriph], 180, 180); %Izquierda
        elseif TR(tr).diskType == 2
            Screen('FillArc', Cfg.windowPtr, Cfg.red,[circ_left,circ_up - diametro - 100,circ_right,circ_down - diametro - 100], 0, 180); %Derecha  - ¡Aquí!
    ...
    if f >= fmask_start && f < (fmask_start + fmask_dur)
        ...
        Screen('DrawTexture', Cfg.windowPtr, faceData, [], [circ_left,circ_up - diametro - 100,circ_right,circ_down - diametro - 100]); % - ¡Aquí!
