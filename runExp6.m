function runExp6(noCST, noPST, noDT, noPDT, RANDOMIZE)
...
%         gray = GrayIndex(w,0.3);
        ...
        diametro = (circ_down - circ_up);
        radio = (diametro/2);
        
        if TR(tr).diskType == 1
            TR(tr).rectCirclePeriph
            Screen('FillArc', Cfg.windowPtr, Cfg.green, [circ_left + radio,circ_up,circ_right + radio,circ_down], 0, 180); %Derecha + 100 - ¡Aquí!
            Screen('FillArc', Cfg.windowPtr, Cfg.red, [TR(tr).rectCirclePeriph], 180, 180); %Izquierda
            Screen('FillRect', Cfg.windowPtr, Cfg.gray, [circ_left + radio,circ_up,circ_right, circ_down])
%             Screen('FillRect', Cfg.windowPtr, Cfg.gray, [circ_left + radio,circ_up,circ_right + 100 - radio, circ_down])

        elseif TR(tr).diskType == 2
            Screen('FillArc', Cfg.windowPtr, Cfg.red,[circ_left + radio,circ_up,circ_right + radio,circ_down], 0, 180); %Derecha  - ¡Aquí!
            Screen('FillArc', Cfg.windowPtr, Cfg.green, [TR(tr).rectCirclePeriph], 180, 180);%Izquierda
            Screen('FillRect', Cfg.windowPtr, Cfg.gray, [circ_left + radio,circ_up,circ_right, circ_down])
%             Screen('FillRect', Cfg.windowPtr, Cfg.gray, [circ_left + radio,circ_up,circ_right + 100 - radio, circ_down])
    ...
    if f >= fmask_start && f < (fmask_start + fmask_dur)
        ...
        Screen('DrawTexture', Cfg.windowPtr, faceData, [], [circ_left + radio,circ_up,circ_right + radio,circ_down]); % - ¡Aquí!
        
        Screen('DrawTexture', Cfg.windowPtr, faceData, [], [circ_left + radio/2,circ_up,circ_right + radio/2, circ_down]); % - ¡Aquí!
